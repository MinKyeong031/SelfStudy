...
void title(void) {
    ...
    gotoxy(MAP_X + (MAP_WIDTH / 2) - 7, MAP_Y + 11, "   ¡Þ ¡ç,¡æ,¡è,¡é : Move    ");
    gotoxy(MAP_X + (MAP_WIDTH / 2) - 7, MAP_Y + 12, "   ¡Þ P : Pause             ");
    gotoxy(MAP_X + (MAP_WIDTH / 2) - 7, MAP_Y + 13, "   ¡Þ ESC : Quit              ");
...
void reset(void) {
    ...
    for (i = 0; i < length; i++) {
        x[i] = MAP_WIDTH / 2 + i;
        y[i] = MAP_HEIGHT / 2;
        gotoxy(MAP_X + x[i], MAP_Y + y[i], "¡Ü");
    }
    gotoxy(MAP_X + x[0], MAP_Y + y[0], "¡Ü"); 
    food(); 
}
...
void food(void) {
    ...
    while (1) {
        ...
        gotoxy(MAP_X + food_x, MAP_Y + food_y, "¡Ú"); 
        speed -= 3;
        break;
